...
def predict():
    if request.method == 'POST':
        ...
        try:
            ...
            if predicted_class in ['The image contains a brain tumor and the type is Glioma', 
                                   'The image contains a brain tumor and the type is Meningioma', 
                                   'The image contains a brain tumor and the type is Pituitary']:
                return render_template('pred.html', show_pip=True, prediction_result=predicted_class)
            else:
                return render_template('pred.html', show_pip=False, prediction_result=predicted_class)
